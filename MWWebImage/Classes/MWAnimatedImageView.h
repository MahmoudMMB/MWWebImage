...
/**
 ...
 Call `setImage:` with `UIImage(NSImage)` which conform to `MWAnimatedImage` protocol will start animated image rendering. Call with normal UIImage(NSImage) will back to normal UIImageView(NSImageView) rendering
 ...
 */
@interface MWAnimatedImageView : UIImageView
...
/**
 ...
 @note The best practice to use incremental load is using `initWithAnimatedCoder:scale:` in `MWAnimatedImage` with animated coder which conform to `MWProgressiveImageCoder` as well. Then call incremental update and incremental decode method to produce the image.
...
/**
 ...
 Default is NSRunLoopCommonModes on multi-core device, NMWefaultRunLoopMode on single-core device
 @note This is useful for some cases, for example, always specify NMWefaultRunLoopMode, if you want to pause the animation when user scroll (for Mac user, drag the mouse or touchpad)
 */
