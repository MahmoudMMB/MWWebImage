...
typedef NS_ENUM(NSUInteger, MWImageScaleMode) {
    MWImageScaleModeFill = 0,
    MWImageScaleModeAspectFit = 1,
    MWImageScaleModeAspectFill = 2
};

#if MW_UIKIT || MW_WATCH
typedef UIRectCorner MWRectCorner;
#else
typedef NS_OPTIONS(NSUInteger, MWRectCorner) {
    MWRectCornerTopLeft     = 1 << 0,
    MWRectCornerTopRight    = 1 << 1,
    MWRectCornerBottomLeft  = 1 << 2,
    MWRectCornerBottomRight = 1 << 3,
    MWRectCornerAllCorners  = ~0UL
};
#endif
 ...
 */
- (nullable UIImage *)MW_roundedCornerImageWithRadius:(CGFloat)cornerRadius
                                              corners:(MWRectCorner)corners
...
/**
 ...
 @note The rect's width/height should not be smaller than or equal to 0. The minX/minY should not be smaller than 0. The maxX/maxY should not be greater than width/height. Attention this limit is different from `MW_colorAtPoint:` (point: (0, 0) like rect: (0, 0, 1, 1))
 @note The overhead of object creation means this method is best suited for infrequent color sampling. For heavy image processing, grab the raw bitmap data and process yourself.
