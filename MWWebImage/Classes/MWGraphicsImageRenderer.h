...
/**
 ...
 For others (macOS/watchOS or iOS/tvOS 10-), these method use the `MWImageGraphics.h` to implements the same behavior (but without dynamic bitmap support)
*/

typedef void (^MWGraphicsImageDrawingActions)(CGContextRef _Nonnull context);
typedef NS_ENUM(NSInteger, MWGraphicsImageRendererFormatRange) {
    MWGraphicsImageRendererFormatRangeUnspecified = -1,
    MWGraphicsImageRendererFormatRangeAutomatic = 0,
    MWGraphicsImageRendererFormatRangeExtended,
    MWGraphicsImageRendererFormatRangeStandard
};

/// A set of drawing attributes that represent the configuration of an image renderer context.
@interface MWGraphicsImageRendererFormat : NSObject
...
@property (nonatomic) MWGraphicsImageRendererFormatRange preferredRange;
...
@interface MWGraphicsImageRenderer : NSObject
...
/// @param size The size of images output from the renderer, specified in points.
/// @param format A MWGraphicsImageRendererFormat object that encapsulates the format used to create the renderer context.
/// @return An initialized image renderer.
- (nonnull instancetype)initWithSize:(CGSize)size format:(nonnull MWGraphicsImageRendererFormat *)format;

/// Creates an image by following a set of drawing instructions.
/// @param actions A MWGraphicsImageDrawingActions block that, when invoked by the renderer, executes a set of drawing instructions to create the output image.
...
- (nonnull UIImage *)imageWithActions:(nonnull NS_NOESCAPE MWGraphicsImageDrawingActions)actions;
