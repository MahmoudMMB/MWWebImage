...
static NSArray *MWBundlePreferredScales() {
...
@implementation MWImageAssetManager {
    dispatch_semaphore_t _lock;
}

+ (instancetype)sharedAssetManager {
    static dispatch_once_t onceToken;
    static MWImageAssetManager *assetManager;
    dispatch_once(&onceToken, ^{
        assetManager = [[MWImageAssetManager alloc] init];
    });
    return assetManager;
}
...
- (NSString *)getPathForName:(NSString *)name bundle:(NSBundle *)bundle preferredScale:(CGFloat *)scale {
    ...
    NSArray *scales = MWBundlePreferredScales();
