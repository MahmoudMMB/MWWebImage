...
@property (nonatomic,strong) MWAnimatedImagePlayer *player; // The animation player.
@property (nonatomic) CALayer *imageViewLayer; // The actual rendering layer.

@end

@implementation MWAnimatedImageView
#if MW_UIKIT
...
+ (instancetype)imageViewWithImage:(NSImage *)image
{
    ...
    MWAnimatedImageView *imageView = [[MWAnimatedImageView alloc] initWithFrame:frame];
...
- (void)setImage:(UIImage *)image
{
    ...
    // We need call super method to keep function. This will impliedly call `setNeedMWisplay`. But we have no way to avoid this when using animated image. So we call `setNeedMWisplay` again at the end.
    super.image = image;
    if ([image.class conformsToProtocol:@protocol(MWAnimatedImage)]) {
        if (!self.player) {
            id<MWAnimatedImageProvider> provider;
            // Check progressive loading
            if (self.isProgressive) {
                provider = [self progressiveAnimatedCoderForImage:image];
            } else {
                provider = (id<MWAnimatedImage>)image;
            }
            // Create animated player
            self.player = [MWAnimatedImagePlayer playerWithProvider:provider];
        } else {
            // Update Frame Count
            self.player.totalFrameCount = [(id<MWAnimatedImage>)image animatedImageFrameCount];
        }
...
- (void)updateIsProgressiveWithImage:(UIImage *)image
{
    ...
    id<MWAnimatedImageCoder> currentAnimatedCoder = [self progressiveAnimatedCoderForImage:image];
    if (currentAnimatedCoder) {
        ...
        if (!previousImage) {
            ...
            id<MWAnimatedImageCoder> previousAnimatedCoder = [self progressiveAnimatedCoderForImage:previousImage];
...
- (id<MWAnimatedImageCoder, MWProgressiveImageCoder>)progressiveAnimatedCoderForImage:(UIImage *)image
{
    if ([image.class conformsToProtocol:@protocol(MWAnimatedImage)] && image.MW_isIncremental && [image respondsToSelector:@selector(animatedCoder)]) {
        id<MWAnimatedImageCoder> animatedCoder = [(id<MWAnimatedImage>)image animatedCoder];
        if ([animatedCoder conformsToProtocol:@protocol(MWProgressiveImageCoder)]) {
            return (id<MWAnimatedImageCoder, MWProgressiveImageCoder>)animatedCoder;
        }
    }
    return nil;
}
...
- (NSView *)imageView {
    NSImageView *imageView = objc_getAssociatedObject(self, MW_SEL_SPI(imageView));
    if (!imageView) {
        // macOS 10.14
        imageView = objc_getAssociatedObject(self, MW_SEL_SPI(imageSubview));
    }
    return imageView;
}
