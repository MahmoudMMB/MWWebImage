 ...
 */
FOUNDATION_EXPORT const int64_t MWWebImageProgressUnitCountUnknown; /* 1LL */

typedef void(^MWSetImageBlock)(UIImage * _Nullable image, NSData * _Nullable imageData, MWImageCacheType cacheType, NSURL * _Nullable imageURL);

/**
 Integrates MWWebImage async downloading and caching of remote images with UIView subclass.
 */
 ...
 */
@property (nonatomic, strong, readonly, nullable) NSURL *MW_imageURL;
 ...
 * See more about this in `MWWebImageContextSetImageOperationKey`.
 ...
 */
@property (nonatomic, strong, readonly, nullable) NSString *MW_latestOperationKey;
 ...
 * The `totalUnitCount` and `completedUnitCount` will be reset to 0 after a new image loading start (change from current queue). And they will be set to `MWWebImageProgressUnitCountUnknown` if the progressBlock not been called but the image loading success to mark the progress finished (change from main queue).
 ...
 */
@property (nonatomic, strong, null_resettable) NSProgress *MW_imageProgress;
 ...
 *
 *   The fifth parameter normally is always YES. However, if you provide MWWebImageAvoidAutoSetImage with MWWebImageProgressiveLoad options to enable progressive downloading and set the image yourself. This block is thus called repeatedly with a partial image. When image is fully downloaded, the
...
/**
 The image transition when image load finished. See `MWWebImageTransition`.
 If you specify nil, do not do transition. Defaults to nil.
 */
@property (nonatomic, strong, nullable) MWWebImageTransition *MW_imageTransition;
 ...
 */
@property (nonatomic, strong, nullable) id<MWWebImageIndicator> MW_imageIndicator;
