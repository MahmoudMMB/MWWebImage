...
+ (instancetype)sharedCoder {
    static MWImageIOCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[MWImageIOCoder alloc] init];
    });
    return coder;
}
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable MWImageCoderOptions *)options {
    ...
    UIImage *image = [MWImageIOAnimatedCoder createFrameAtIndex:0 source:source scale:scale preserveAspectRatio:preserveAspectRatio thumbnailSize:thumbnailSize options:nil];
    CFRelease(source);
    if (!image) {
        return nil;
    }
    
    image.MW_imageFormat = [NSData MW_imageFormatForImageData:data];
    return image;
}
...
- (UIImage *)incrementalDecodedImageWithOptions:(MWImageCoderOptions *)options {
    ...
    if (_width + _height > 0) {
        ...
        image = [MWImageIOAnimatedCoder createFrameAtIndex:0 source:_imageSource scale:scale preserveAspectRatio:_preserveAspectRatio thumbnailSize:_thumbnailSize options:nil];
        if (image) {
            CFStringRef uttype = CGImageSourceGetType(_imageSource);
            image.MW_imageFormat = [NSData MW_imageFormatFromUTType:uttype];
        }
    }
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(MWImageFormat)format options:(nullable MWImageCoderOptions *)options {
    ...
    if (format == MWImageFormatUndefined) {
        BOOL hasAlpha = [MWImageCoderHelper CGImageContainsAlpha:imageRef];
        if (hasAlpha) {
    ...
    CGImagePropertyOrientation exifOrientation = [MWImageCoderHelper exifOrientationFromImageOrientation:image.imageOrientation];
#else
