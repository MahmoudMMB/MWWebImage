...
static inline CGRect MWCGRectFitWithScaleMode(CGRect rect, CGSize size, MWImageScaleMode scaleMode) {
    rect = CGRectStandardize(rect);
    size.width = size.width < 0 ? -size.width : size.width;
    size.height = size.height < 0 ? -size.height : size.height;
    CGPoint center = CGPointMake(CGRectGetMidX(rect), CGRectGetMidY(rect));
    switch (scaleMode) {
        case MWImageScaleModeAspectFit:
        case MWImageScaleModeAspectFill: {
            if (rect.size.width < 0.01 || rect.size.height < 0.01 ||
                ...
                CGFloat scale;
                if (scaleMode == MWImageScaleModeAspectFit) {
...
static inline UIColor * MWGetColorFromPixel(Pixel_8888 pixel, CGBitmapInfo bitmapInfo) {
...
static inline CGImageRef _Nullable MWCreateCGImageFromCIImage(CIImage * _Nonnull ciImage) {
...
- (void)MW_drawInRect:(CGRect)rect context:(CGContextRef)context scaleMode:(MWImageScaleMode)scaleMode clipsToBounds:(BOOL)clips {
    CGRect drawRect = MWCGRectFitWithScaleMode(rect, self.size, scaleMode);
...
- (nullable UIImage *)MW_resizedImageWithSize:(CGSize)size scaleMode:(MWImageScaleMode)scaleMode {
    if (size.width <= 0 || size.height <= 0) return nil;
    MWGraphicsImageRendererFormat *format = [[MWGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    MWGraphicsImageRenderer *renderer = [[MWGraphicsImageRenderer alloc] initWithSize:size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
        [self MW_drawInRect:CGRectMake(0, 0, size.width, size.height) context:context scaleMode:scaleMode clipsToBounds:NO];
    }];
    return image;
}

- (nullable UIImage *)MW_croppedImageWithRect:(CGRect)rect {
...
- (nullable UIImage *)MW_roundedCornerImageWithRadius:(CGFloat)cornerRadius corners:(MWRectCorner)corners borderWidth:(CGFloat)borderWidth borderColor:(nullable UIColor *)borderColor {
    MWGraphicsImageRendererFormat *format = [[MWGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    MWGraphicsImageRenderer *renderer = [[MWGraphicsImageRenderer alloc] initWithSize:self.size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
...
#else
            NSBezierPath *path = [NSBezierPath MW_bezierPathWithRoundedRect:CGRectInset(rect, borderWidth, borderWidth) byRoundingCorners:corners cornerRadius:cornerRadius];
#endif
            [path closePath];
...
#else
            NSBezierPath *path = [NSBezierPath MW_bezierPathWithRoundedRect:strokeRect byRoundingCorners:corners cornerRadius:strokeRadius];
#endif
            [path closePath];
...
- (nullable UIImage *)MW_rotatedImageWithAngle:(CGFloat)angle fitSize:(BOOL)fitSize {
    ...
    MWGraphicsImageRendererFormat *format = [[MWGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    MWGraphicsImageRenderer *renderer = [[MWGraphicsImageRenderer alloc] initWithSize:newRect.size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
...
- (nullable UIImage *)MW_flippedImageWithHorizontal:(BOOL)horizontal vertical:(BOOL)vertical {
    size_t width = self.size.width;
    size_t height = self.size.height;
...
- (nullable UIImage *)MW_tintedImageWithColor:(nonnull UIColor *)tintColor {
...
- (nullable UIColor *)MW_colorAtPoint:(CGPoint)point {
    CGImageRef imageRef = NULL;
    // CIImage compatible
#if MW_UIKIT || MW_MAC
    if (self.CIImage) {
        imageRef = MWCreateCGImageFromCIImage(self.CIImage);
    }
#endif
    if (!imageRef) {
    ...
    CGImageRelease(imageRef);
    // Convert to color
    return MWGetColorFromPixel(pixel, bitmapInfo);
}

- (nullable NSArray<UIColor *> *)MW_colorsWithRect:(CGRect)rect {
    CGImageRef imageRef = NULL;
    // CIImage compatible
#if MW_UIKIT || MW_MAC
    if (self.CIImage) {
    ...
    for (size_t index = start; index < end; index += 4) {
        ...
        UIColor *color = MWGetColorFromPixel(pixel, bitmapInfo);
...
- (nullable UIImage *)MW_blurredImageWithRadius:(CGFloat)blurRadius {
    ...
    if (CGImageGetBitsPerPixel(imageRef) != 32 ||
        ...
        MWGraphicsBeginImageContextWithOptions(self.size, NO, self.scale);
        [self drawInRect:CGRectMake(0, 0, self.size.width, self.size.height)];
        imageRef = MWGraphicsGetImageFromCurrentImageContext().CGImage;
...
#if MW_UIKIT || MW_MAC
- (nullable UIImage *)MW_filteredImageWithFilter:(nonnull CIFilter *)filter {
    CIImage *inputImage;
    if (self.CIImage) {
