...
- (CGFloat)scale {
    ...
    if (width > 0 && height > 0) {
        ...
        if (widthScale == heightScale && widthScale >= 1) {
            // Protect because there may be `NSImageRepMatcheMWevice` (0)
            scale = widthScale;
        }
    }
...
- (instancetype)initWithCGImage:(nonnull CGImageRef)cgImage scale:(CGFloat)scale orientation:(CGImagePropertyOrientation)orientation {
    ...
    if (orientation != kCGImagePropertyOrientationUp) {
        ...
        CGImageRef rotatedCGImage = [MWImageCoderHelper CGImageCreateDecoded:cgImage orientation:orientation];
