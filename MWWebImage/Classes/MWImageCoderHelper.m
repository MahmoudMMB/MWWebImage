...
static inline size_t MWByteAlign(size_t size, size_t alignment) {
...
/*
 * Defines the maximum size in MB of the decoded image when the flag `MWWebImageScaleDownLargeImages` is set
...
@implementation MWImageCoderHelper

+ (UIImage *)animatedImageWithFrames:(NSArray<MWImageFrame *> *)frames {
    ...
    [frames enumerateObjectsUsingBlock:^(MWImageFrame * _Nonnull frame, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    CFStringRef imageUTType = [NSData MW_UTTypeFromImageFormat:MWImageFormatGIF];
    ...
    MWAnimatedImageRep *imageRep = [[MWAnimatedImageRep alloc] initWithData:imageData];
...
+ (NSArray<MWImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
    if (!animatedImage) {
        return nil;
    }
    ...
    [animatedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull image, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([image isEqual:previousImage]) {
            ...
            MWImageFrame *frame = [MWImageFrame frameWithImage:previousImage duration:avgDuration * repeatCount];
    ...
    for (size_t i = 0; i < frameCount; i++) {
        @autoreleasepool {
            ...
            MWImageFrame *frame = [MWImageFrame frameWithImage:frameImage duration:frameDuration];
            [frames addObject:frame];
        }
    }
#endif
...
+ (CGImageRef)CGImageCreateDecoded:(CGImageRef)cgImage orientation:(CGImagePropertyOrientation)orientation {
    ...
    // Apply transform
    CGAffineTransform transform = MWCGContextTransformFromOrientation(orientation, CGSizeMake(newWidth, newHeight));
...
+ (CGImageRef)CGImageCreateScaled:(CGImageRef)cgImage size:(CGSize)size {
    ...
    output_buffer.rowBytes = MWByteAlign(output_buffer.width * 4, 64);
...
+ (UIImage *)decodedImageWithImage:(UIImage *)image {
    ...
    MWImageCopyAssociatedObject(image, decodedImage);
    decodedImage.MW_iMWecoded = YES;
    return decodedImage;
}
...
+ (UIImage *)decodedAndScaledDownImageWithImage:(UIImage *)image limitBytes:(NSUInteger)bytes {
    ...
    // on iOS7, do not forget to call [[MWImageCache sharedImageCache] clearMemory];
    @autoreleasepool {
        ...
        MWImageCopyAssociatedObject(image, destImage);
        destImage.MW_iMWecoded = YES;
        return destImage;
    }
}
...
static inline CGAffineTransform MWCGContextTransformFromOrientation(CGImagePropertyOrientation orientation, CGSize size) {
